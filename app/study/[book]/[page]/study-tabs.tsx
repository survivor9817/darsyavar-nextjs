...
import Fehrest from "@/components/fehrest/fehrest";
import Book from "@/components/book/book";
import Quiz from "@/components/quiz/quiz";
import Yavar from "@/components/yavar/yavar";
...
const StudyTabs = ({ bookId, page }: StudyTabsProps) => {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  const rawValue = searchParams.get("tab");
  const isValidTab = TABS.some((tab) => tab.value === rawValue);
  const value = isValidTab ? (rawValue as (typeof TABS)[number]["value"]) : DEFAULT_TAB;
  const activeIndex = TABS.findIndex((tab) => tab.value === value);

  useEffect(() => {
    const handleIncorrectTabUrl = () => {
      const params = new URLSearchParams(searchParams);
      params.set("tab", DEFAULT_TAB);
      router.replace(`${pathname}?${params.toString()}`, { scroll: false });
    };

    if (!isValidTab) handleIncorrectTabUrl();
  }, [isValidTab, pathname, router, searchParams]);
  ...
  return (
    <div className="w-full max-w-210 min-w-80 h-vh h-dvh mx-auto overflow-hidden flex">
      <Tabs.Root
        ...
        value={value}
        onValueChange={(newValue) => handleChange(newValue as string)}
      >
        {/* Tab List */}
